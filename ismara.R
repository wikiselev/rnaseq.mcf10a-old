...
process_tf_data <- function(file.name, sig.table) {
    ...
    x <- x[c(1, 7, 13, 2, 8, 14, 5, 11, 17, 6, 12, 18, 3, 9, 15, 4, 10, 16, 19, 25, 31, 20, 26, 32, 23, 29, 
        35, 24, 30, 36, 21, 27, 33, 22, 28, 34, 37, 38, 39, 40, 46, 52, 41, 47, 53, 44, 50, 56, 45, 51, 57, 
        42, 48, 54, 43, 49, 55, 58, 68, 70, 71, 59, 64, 69, 62, 75, 74, 63, 67, 72, 60, 65, 61, 66, 73), ]
    v <- v[x$index, ]
...
plot_ismara_activities <- function() {
    # import table with TF motifs' significance values note that I modified this file a bit before importing
    ...
    p <- ggplot(data, aes(time, mean, color = cond, group = group)) + geom_line() + facet_wrap(~ind, ncol = 10) + 
...
plot_ismara_averaged_activities <- function() {
    ...
    # plot all TF motifs activities in one pdf file
    p <- ggplot(data, aes(time, value, color = cond, group = group)) + geom_line() + facet_wrap(~variable, 
        ncol = 10) + geom_errorbar(limits, width = 0.25)
...
get_ismara_target_info_from_file_new <- function(file) {
    ...
    mytable <- unique(data.table(location = rep(loc, len), score = rep(score, len), motif = rep(motif, len), 
        target = target, ref = ref))
    
    # remove all GenBank entries, leave only RefSeq entries and collaps entries with the same
    # location,score,motif,target sets
...
ismara_plot_score_dist <- function(data, name) {
    ...
    p <- ggplot(data, aes(x = score)) + geom_bar(aes(fill = diff), position = "dodge", width = 0.3) + facet_wrap(~motif, 
        scale = "free_y") + scale_fill_manual(values = cbPalette) + labs(x = "Target score, Sm", y = "Target number") + 
        guides(fill = guide_legend(title = "Gene types")) + theme_bw()
    # theme(axis.line = element_blank(), panel.grid.major = element_blank(), panel.grid.minor =
    # element_blank(), panel.border = element_blank(), panel.background = element_blank()) scale_x_log10()
    pdf(file = paste0("../pip3-rna-seq-output/figures/motif-diff-activity-", name, ".pdf"), w = 16, h = 10)
    print(p)
    dev.off()
}
...
motif_diff_activity_new <- function(gene.set1, gene.set2, diff1, diff2, name) {
    ...
    ks <- d[, list(D = kolmogorov_smirnov_test(score, diff, diff1, diff2)[[1]], p = kolmogorov_smirnov_test(score, 
        diff, diff1, diff2)[[2]]), by = "motif"]
    
    ks <- ks[order(-ks[, D])]
    # ks <- ks[p < 0.2]
    
    wx <- d[, list(W = wilcox_test(score, diff, diff1, diff2)[[1]], p = wilcox_test(score, diff, diff1, diff2)[[3]]), 
        by = "motif"]
...
arrange_ismara_activity_matrix <- function(count.matrix) {
    x <- c(37, 38, 39, 58, 68, 70, 59, 64, 71, 62, 69, 75, 63, 67, 74, 60, 65, 72, 61, 66, 73, 19, 25, 31, 
        20, 26, 32, 23, 29, 35, 24, 30, 36, 21, 27, 33, 22, 28, 34, 1, 7, 13, 2, 8, 14, 5, 11, 17, 6, 12, 
        18, 3, 9, 15, 4, 10, 16, 40, 46, 52, 41, 47, 53, 44, 50, 56, 45, 51, 57, 42, 48, 54, 43, 49, 55)
