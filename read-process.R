count_matrix_from_files <- function(files, normalized) {
    # this function create a count matrix from raw htseq read counts files it uses DESeq2 library to construct
    # the matrix
    
    # create a design matrix for DESeq2
    sample.name <- sapply(strsplit(files, "_trimmed"), "[[", 1)
    condition <- sapply(strsplit(sample.name, "_"), "[[", 1)
    samples <- data.frame(sample.name = sample.name, file.name = paste0("../pip3-rna-seq-input/htseq-read-count-raw/", 
    ...
    cds <- DESeqDataSetFromHTSeqCount(sampleTable = samples, directory = ".", design = ~condition)
    
    # if normalized is TRUE then the count matrix is normalized by library size using estimateSizeFactors
    # function of DESeq2
    if (normalized) 
        {
...
arrange_count_matrix_columns <- function(count.matrix) {
    # rearrage columns in count matrix so that they ordered by treatments and times if count matrix is without
    # bad replicates - there are 75 samples
    if (dim(count.matrix)[2] == 75) {
        x <- c(1, 7, 13, 2, 8, 14, 5, 11, 17, 6, 12, 18, 3, 9, 15, 4, 10, 16, 19, 25, 31, 20, 26, 32, 23, 
            29, 35, 24, 30, 36, 21, 27, 33, 22, 28, 34, 37, 38, 39, 40, 46, 52, 41, 47, 53, 44, 50, 56, 45, 
            51, 57, 42, 48, 54, 43, 49, 55, 58, 73, 68, 59, 64, 69, 62, 74, 75, 63, 67, 72, 60, 65, 70, 61, 
            66, 71)
    } else {
        # with bad replicates - there are 78 samples
        x <- c(1, 7, 13, 2, 8, 14, 5, 11, 17, 6, 12, 18, 3, 9, 15, 4, 10, 16, 19, 25, 31, 20, 26, 32, 23, 
            29, 35, 24, 30, 36, 21, 27, 33, 22, 28, 34, 37, 38, 39, 40, 46, 52, 41, 47, 53, 44, 50, 56, 45, 
            51, 57, 42, 48, 54, 43, 49, 55, 58, 64, 70, 59, 65, 71, 62, 68, 74, 63, 69, 75, 60, 66, 72, 61, 
            67, 73, 76, 77, 78)
    }
...
count_matrix_2_data_table <- function(m) {
    # this function create a data table from a read count matrix data tables are useful for some analysis
    d <- as.data.frame(m)
...
ave_repl <- function(count.matrix, name) {
    ...
    saveRDS(res[[1]], paste0("../pip3-rna-seq-output/rds/count-matrix-av", name, ".rds"))
    saveRDS(res[[2]], paste0("../pip3-rna-seq-output/rds/count-matrix-sd", name, ".rds"))
    
    plot.data.all <- create_plot_table(res[[1]], res[[2]])
    saveRDS(plot.data.all, paste0("../pip3-rna-seq-output/rds/plot-time-courses-all-genes", name, ".rds"))
}
...
pca <- function() {
    ...
    p <- ggplot(data, aes(PC1, PC2, color = cond)) + geom_point(aes(size = time, shape = cond)) + scale_size(range = c(3, 
        6)) + theme_bw()
    pdf(file = "../pip3-rna-seq-output/figures/pca12.pdf", w = 7, h = 6)
    print(p)
    dev.off()
    
    p <- ggplot(data, aes(PC2, PC3, color = cond)) + geom_point(aes(size = time, shape = cond)) + scale_size(range = c(3, 
        6)) + theme_bw()
    pdf(file = "../pip3-rna-seq-output/figures/pca23.pdf", w = 7, h = 6)
    print(p)
    dev.off()
    
    p <- ggplot(data, aes(PC1, PC3, color = cond)) + geom_point(aes(size = time, shape = cond)) + scale_size(range = c(3, 
        6)) + theme_bw()
