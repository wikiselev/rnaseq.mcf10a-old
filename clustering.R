# hier_clust <- function(genes) {
# 	cm <- readRDS("../pip3-rna-seq-output/rds/count-matrix-scaled.rds")
# 	# select genes of interest
# 	cm <- cm[rownames(cm) %in% genes,]
# 	h.c <- hclust(dist(cm), method = "complete")
# }

# k_means_clust <- function(genes, clust.num, name) {
# 	set.seed(2)
# 	cm <- readRDS("../pip3-rna-seq-output/rds/count-matrix-scaled.rds")
# 	# select genes of interest
# 	cm <- cm[rownames(cm) %in% genes,]
# 	k.m <- kmeans(cm, clust.num, nstart = 20)
# 	plot_clust_profile(k.m$cluster, paste0(name, "-k-m-", clust.num))
# 	saveRDS(k.m, paste0("../pip3-rna-seq-output/rds/", name, "-k-m-", clust.num, ".rds"))
# 	# return(k.m)
# }
...
clust_boot <- function(genes, min.clust.num, max.clust.num, name, ind) {
	...
	for(i in min.clust.num:max.clust.num){
		c.boot <- clusterboot(cm, B = 100, bootmethod="boot",
...
plot_bootstrap_data <- function(name) {
	...
	pdf(file = paste0("../pip3-rna-seq-output/figures/cluster-", name, "-boots.pdf"),
	  width = 12,
	  height = 8)
	print(p)
	dev.off()
...
# plot_clust_profile <- function(clusts, name) {
...
# 	# p <- ggplot(plot.data,
# 	#   aes(time, value, group = gr, color = cond)) +
# 	#     geom_line() + facet_wrap(~ clust, scale = "free_y")

# 	# pdf(file = paste0("../pip3-rna-seq-output/figures/clusts-", name, ".pdf"),
# 	#   width = (sqrt(length(unique(clusts))) + 2)*3,
# 	#   height = sqrt(length(unique(clusts)))*3)
# 	# print(p)
# 	# dev.off()

# 	count.matrix <- readRDS("../pip3-rna-seq-output/rds/count-matrix-scaled.rds")
# 	heatmap.data <- count.matrix[rownames(count.matrix) %in% names(clusts), ]
# 	heatmap.data <- heatmap.data[order(clusts),]
# 	pdf(file = paste0("../pip3-rna-seq-output/figures/clusts-", name, "-heatmap.pdf"),
# 		w = 10, h = 10)
# 	heatmap.2(heatmap.data, Rowv = FALSE, Colv = FALSE, dendrogram = "none",
# 	  col=bluered(99), breaks = 100, trace = "none")
# 	dev.off()

# 	plot.data1 <-
# 		plot.data[, list(val = mean(value), sd = sd(value)), by = c("cond", "time", "clust")]

# 	limits <- aes(ymax = val + sd, ymin = val - sd)

# 	p <- ggplot(plot.data1,
# 	  aes(time, val, group = cond, color = cond)) +
# 	    geom_line() + facet_wrap(~ clust, scale = "free_y")
# 	    # geom_errorbar(limits, width = 0.25)
...
plot_all_clusts <- function(clusts, name, inds) {
	...
	pdf(file = paste0("../pip3-rna-seq-output/figures/clusts-", name, "-av-all.pdf"),
	  width = 10,
	  height = 10)
	print(p)
	dev.off()
