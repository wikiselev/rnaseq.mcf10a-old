diff_expr_pairwise <- function(cond1, cond2, time1, time2) {
  ...
  # change levels
  samples$condition <-
    factor(samples$condition, levels=condition.levels)

  # create a DESeqDataSet with interaction term in the design formula
  cds <- DESeqDataSetFromHTSeqCount(sampleTable = samples,
    directory = "../pip3-rna-seq-input/htseq-read-count-deseq/",
    design = ~ condition)

  # perform the main computation
  cds <- DESeq(cds)

  saveRDS(results(cds), paste0("../pip3-rna-seq-output/rds/diff-expr-", cond1,
    "-", time1, "-", cond2, "-", time2, ".rds"))
}

diff_expr_pairwise_mirna <- function(cond1, cond2, time1, time2) {
  matrix <- readRDS("../pip3-rna-seq-output/rds/count-matrix-mirna.rds")
  matrix <- matrix[,grepl(paste0(cond1, "_", ".*", "_", time1), colnames(matrix)) | 
                    grepl(paste0(cond2, "_", ".*", "_", time2), colnames(matrix))]
  ...
  # change levels
  colData$condition <-
    factor(colData$condition, levels=condition.levels)
  ...
  saveRDS(results(dds), paste0("../pip3-rna-seq-output/rds/diff-expr-mirna-",
    cond1, "-", time1, "-", cond2, "-", time2, ".rds"))
}
...
diff_expr_time_course_mirna <- function(cond) {
  ...
  saveRDS(results(ddsLRT), paste0("../pip3-rna-seq-output/rds/diff-expr-mirna-",
    cond, ".rds"))
}
...
diff_expr_time_course <- function(cond) {
  ...
  # create a DESeqDataSet with interaction term in the design formula
  cds <- DESeqDataSetFromHTSeqCount(sampleTable = samples,
    directory = "../pip3-rna-seq-input/htseq-read-count-deseq/", design = ~ time)

  # perform the main computation
  cds <- DESeq(cds)

  # using a new feature in DESeq2 package
  # use maximum likelihood test to find diff. expressed genes in WT time course
  cdsLRT <- nbinomLRT(cds, reduced = ~ 1)
  saveRDS(results(cdsLRT), paste0("../pip3-rna-seq-output/rds/diff-expr-",
    cond, ".rds"))
}
...
diff_expr_two_time_courses <- function(cond1, cond2) {
  ...
  files <- files[
    grepl(
      paste(c(paste0(cond1, "_"), paste0(cond2, "_")), collapse = "|"), files)]
  time <- sapply(strsplit(files, "\\_"), "[[", 3)
  condition <- sapply(strsplit(files, "\\_"), "[[", 1)
  samples <- data.frame(sample.name = files, file.name = files,
