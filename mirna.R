...
mirna_read_process <- function() {
    ...
    count.matrix <- read.delim("../pip3-rna-seq-input/miRNA/summarised_mirna_counts.txt", row.names = 1)
...
plot_mirna <- function(mirnas, s) {
    # if genes are represented by ensembl ids if(grepl('ENSG000', genes[1])){ res <-
    # ensembl_id_to_hgnc_symbol(genes) } else { res <- hgnc_symbol_to_ensembl_id(genes) } colnames(res) <-
    # c('id', 'name')
    ...
    p <- ggplot(plot.data, aes(time, value, group = cond, color = cond)) + geom_line() + facet_wrap(~id, scale = "free_y") + 
        geom_errorbar(limits, width = 0.25)
    pdf(file = paste0("../pip3-rna-seq-output/figures/mirna-", s, ".pdf"), width = (sqrt(length(mirnas)) + 
        2) * 3, height = sqrt(length(mirnas)) * 3)
    print(p)
    dev.off()
} 
